...
public class CamelizeControllerTest {
    ...
    public void testCamelize() throws Exception {
        this.mockMvc.perform(get("/camelize?original=this_is_a_thing").accept(MediaType.TEXT_PLAIN))
                .andExpect(status().isOk())
                .andExpect(content().string("thisIsAThing"));
        this.mockMvc.perform(get("/camelize?original=this_is_a_thing&initialCap=true").accept(MediaType.TEXT_PLAIN))
                .andExpect(status().isOk())
                .andExpect(content().string("ThisIsAThing"));
    }
}
